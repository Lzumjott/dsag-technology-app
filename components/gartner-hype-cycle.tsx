...
const phaseDescriptions: Record<GartnerPhase, string> = {
	"Innovation Trigger": "Früher Technologietrend, erste Proof-of-Concepts. Noch keine marktfähigen Produkte.",
	"Peak of Inflated Expectations": "Überhöhte Erwartungen, erste Erfolgsgeschichten aber auch viele Misserfolge.",
	"Trough of Disillusionment": "Ernüchterung nach gescheiterten Pilotprojekten. Nur beharrliche Anbieter überleben.",
	"Slope of Enlightenment": "Realistische Anwendungsfälle kristallisieren sich heraus. Best Practices entstehen.",
	"Plateau of Productivity": "Breite Marktakzeptanz. Klare ROI-Nachweise und bewährte Implementierungsmuster.",
}
...
export function GartnerHypeCycle() {
	...
	return (
		<section id="gartner" className="grid-section scroll-mt-20 border-t border-border/50 py-[var(--grid-unit)]">
			<div className="grid-content">
				<div className="mb-[var(--grid-unit)]">
					...
					<p className="max-w-2xl text-sm text-muted-foreground sm:text-base">
						Alle {trends.length} Technologietrends eingeordnet in die fünf Phasen des Gartner Hype Cycle.
					</p>
				</div>

				{/* Visual Hype Cycle Curve - Desktop */}
				<div className="mb-8 hidden overflow-hidden border border-border/50 bg-card p-[var(--grid-unit-eighth)] md:block">
				...
				<div className="flex flex-col gap-0">
					{groupedTrends.map(({ phase, trends: phaseTrends }) => (
						<div
							key={phase}
							className={cn(
								"border bg-card p-[var(--grid-unit-eighth)] transition-all",
						...
						>
							<div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-3">
								...
								<span className="bg-secondary px-2 py-0.5 text-[10px] text-muted-foreground sm:text-xs">
							...
							<div className="no-scrollbar flex flex-wrap gap-0">
								{phaseTrends.map((trend) => (
									<div
										key={trend.id}
										className={cn(
											"inline-flex items-center gap-1.5 border px-2.5 py-1.5 text-[11px] font-medium transition-colors sm:text-[12px]",
